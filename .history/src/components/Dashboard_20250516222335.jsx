    ...
    const loadUserAndEvents = async () => {
      try {
        ...
        if (!storedUserToken) {
          ...
          try {
            // 1. Split the token into its three parts: header, payload, signature.
            ...
            // 2. Decode the base64-encoded payload.
            const decodedPayload = atob(payloadBase64.replace(/-/g, '+').replace(/_/g, '/')); // Replace URL-safe characters

            // 3. Parse the decoded payload as JSON.
            const parsedPayload = JSON.parse(decodedPayload);

            // 4. Extract the email from the parsed payload.
            const userEmail = parsedPayload.email; // adjust key name if needed

            if (userEmail) {
              console.log('Logged in as user email:', userEmail);
              ...
              >
                ...
                <button
                  ...
                  style={{
                    ...
                    border: 'none',
                    cursor:
                      event.remainingTickets <= 0 || isBooked ? 'not-allowed' : 'pointer',
                  }}
                >
