      ...
      try {
        ...
        if (!storedUser) {
          ...
          try {
            ...
            if (parsedUser && parsedUser._id) { // Check for both parsedUser and _id
              console.log('Logged in as user:', parsedUser);
              setUser(parsedUser); // Store the entire user object
            } else {
              console.warn('Invalid user object in localStorage:', storedUser); //Show the raw storedUser
              setUser(null); // Ensure user is null to reflect login state
            }
          } catch (error) {
            console.error('Error parsing user object in localStorage:', error);
