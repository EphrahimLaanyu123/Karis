...
const Dashboard = () => {
  ...
  const [user, setUser] = useState(null); // Will hold {_id, email}
      ...
      try {
        ...
        if (!storedUserToken) {
          ...
          try {
            ...
            const userId = parsedPayload._id || parsedPayload.id;

            if (userEmail && userId) {
              setUser({ email: userEmail, _id: userId });
            } else {
              console.warn('Email or ID not found in token payload', parsedPayload);
              setUser(null);
            }
          } catch (error) {
  ...
  const handleBookTicket = async (eventId) => {
    ...
    const numberOfTickets = ticketCounts[eventId] || 1;

    if (numberOfTickets <= 0) {
      setBookingStatus((prev) => ({
        ...prev,
        [eventId]: 'Please enter a valid number of tickets.',
      }));
      return;
    }
    ...
    try {
      const res = await fetch(`http://localhost:3000/api/attendees/book`, {
        ...
        body: JSON.stringify({
          userId: user._id, // Corrected: send ObjectId instead of email
          eventId,
          numberOfTickets,
        }),
      });
      ...
      if (res.ok) {
        ...
        await fetchEvents(); // Refresh ticket counts
        ...
        <ul style={{ listStyleType: 'none', padding: 0 }}>
          {events.map((event) => (
            ...
            >
              ...
              <p>
                <strong>Available Tickets:</strong> {event.tickets}
              </p>
              <div>
                <input
                  type="number"
