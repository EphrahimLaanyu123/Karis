...
const Dashboard = () => {
  ...
  const [ticketCounts, setTicketCounts] = useState({}); // To store desired ticket counts
  ...
  const handleBookTicket = async (eventId) => {
    ...
    try {
      const res = await fetch(`http://localhost:3000/api/attendees/book`, { // Changed route to attendees/book
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          userId: storedUserToken, //  Send user identifier (email in this case)
          eventId: eventId,
      ...
      if (res.ok) {
        ...
        await fetchEvents(); // Refresh events to update ticket counts
          ...
          {events.map((event) => {
            return (
              ...
              >
                ...
                <p>
                  <strong>Total Tickets:</strong> {event.tickets}<br />
                </p>
                <div>
                    <input
                      type="number"
