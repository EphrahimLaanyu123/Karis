...
function AdminNavbar() {
    ...
    useEffect(() => {
        const loadUserAndEvents = async () => {
          try {
            const storedUserToken = localStorage.getItem('adminToken');
    ...
    <>
        <nav className='flex items-center justify-between bg-[#0a3882] py-6 px-6'>
            <div className='flex gap-2 items-center'>
                <Link to={'/admin/dashboard'} className='text-3xl'>Kariz<span className='text-blue-400 font-black'>Events</span></Link> |
                <span>
                {user ? `Signed in as an admin` : 'Admin Dashboard'}
                </span>
            </div>

            <ul className='flex items-center justify-between gap-4 text-lg'> 
                <NavLink to='/create_event' className='text-blue-300 active:text-blue-100 active:border-b-blue-200'>Create an Event</NavLink>
